 ...
 *
 * @param array<mixed>|string $string     String or array to be split.
 * @param array<mixed>|string $delimiters Delimiter(s) to search for.
...
function explode_array(string|array $string, array|string $delimiters = [',']): array
{
    return array_reduce(
        (array) $delimiters,
        static fn (array $arr, string $delimiter): array =>
            array_merge(
                [],
                ...array_map(
 ...
 *
 * @param array<mixed> $a An array that be compacted.
 * @return array<mixed>
 */
 ...
 *
 * @param array<mixed> $array Array of data.
 */
        ...
        'between' => (function () use ($key, $pattern): bool {
            if (! is_array($pattern) || count($pattern) !== 2) {
                throw new TypeException(
                    message: "Query 'between' needs exactly 2 items in array."
                );
            }

            return $key >= $pattern[0] && $key <= $pattern[1];
        })(),
 ...
 *
 * @param array<mixed> $a First item for comparison. The compared items should be
 *                     associative arrays that optionally include a 'Name' key.
 * @param array<mixed> $b Second item for comparison.
